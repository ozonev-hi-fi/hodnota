...
- [ ] **Backend scaffold** — `global.json`, `hodnota.slnx` (or `.sln` fallback), `Directory.Packages.props`, `Directory.Build.props`, root `.gitignore` additions (`bin/`, `obj/`, `.vs/`, `*.user`); `/src` projects (`Hodnota.Domain`, `.Application`, `.Infrastructure`, `.Contracts`, `.Api`) wired with the reference chain from 0003; `/tests` projects (one per `/src` project except `.Contracts`) with a placeholder test each. Verify: `dotnet build` and `dotnet test` both succeed.
